...
// Validation function
export function validate(data: any, schema: ValidationSchema, path: string = ''): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
    ...
    switch (rule.type) {
      case 'string':
        ...
        if (rule.min && value.length < rule.min) {
          errors.push(`${fieldPath} debe tener al menos ${rule.min} caracteres`);
        }
  ...
  function validateObject(obj: any, schema: ValidationSchema, currentPath: string): void {
...
export function validationMiddleware(schema: ValidationSchema, target: 'body' | 'query' | 'params' = 'body') {
...
export const schemas = {
  ...
  updateProfile: {
    ...
    profile: {
      ...
      experienceLevel: { type: 'string' as const, enum: ['beginner', 'intermediate', 'advanced'] },
  ...
  generateRoutine: {
    ...
    experienceLevel: { type: 'string' as const, required: true, enum: ['beginner', 'intermediate', 'advanced'] },
    availableDays: { type: 'number' as const, required: true, min: 1, max: 7 },
    sessionDuration: { type: 'number' as const, required: true, min: 15, max: 180 },
  ...
  // Health sync
  healthSync: {
    dataType: { type: 'string' as const, required: true, enum: ['metrics', 'workouts', 'sleep', 'hrv'] },
