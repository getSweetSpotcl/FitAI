...
import { mockSql, mockUser } from "../test/mocks/database.mock";
import users from "./users";

// Mock database module
vi.mock("../db/database", () => ({
  createDatabaseClient: vi.fn(() => mockSql),
  getUserByClerkId: vi.fn(async (_sql: any, clerkId: string) => {
    if (clerkId === "clerk_test_123") {
      return mockUser;
    }
    return null;
  }),
  getUserById: vi.fn(async (_sql: any, userId: string) => {
...
describe("Users Routes", () => {
  ...
  beforeEach(() => {
    ...
    // Add mock auth middleware that also sets env
    app.use("*", async (c, next) => {
      // Mock environment variables in context
      c.env = {
  ...
  describe("GET /api/v1/users/me/progress", () => {
    it("should return user progress metrics", async () => {
      ...
      const res = await app.request(
        "/api/v1/users/me/progress?period=last_30_days",
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
