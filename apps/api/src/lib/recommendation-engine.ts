...
export class RecommendationEngine {
  ...
  async generateExerciseSubstitutions(
    ...
    const _substitutions: Exercise[] = [];

    // Filter constraints relevant to exercise selection
    const equipmentConstraints = userConstraints.filter(
      (c) => c.type === "equipment"
    );
    const injuryConstraints = userConstraints.filter(
      (c) => c.type === "injury"
    );
    ...
    const filteredExercises = sameMuscleExercises.filter((exercise) => {
      ...
      if (equipmentConstraints.length > 0) {
        const hasRequiredEquipment = exercise.equipment.some(
          (eq) =>
    ...
    const rankedSubstitutions = filteredExercises
      .map((exercise) => ({
        exercise,
        similarity: this.calculateExerciseSimilarity(
          originalExercise,
          exercise
        ),
        suitability: this.calculateExerciseSuitability(
          exercise,
          userConstraints
        ),
      }))
      .sort(
        (a, b) => b.similarity + b.suitability - (a.similarity + a.suitability)
      )
  ...
  async optimizeWorkoutTiming(
    ...
    const optimalTimes = this.findOptimalWorkoutTimes(
      userSchedule,
      timePerformanceMap
    );

    // Generate weekly schedule
    const weeklyPlan = this.generateOptimalWeeklyPlan(
      optimalTimes,
      performanceData
    );
  ...
  async recommendDeloadWeek(
    fatigueMarkers: PerformanceMetrics[]
  ...
  async generateWorkoutRecommendation(
    ...
    const recommendedExercises = await this.prescribeExercises(
      selectedExercises,
      userProfile
    );
    ...
    const estimatedDuration =
      this.calculateWorkoutDuration(recommendedExercises);
    const difficulty = this.calculateWorkoutDifficulty(
      recommendedExercises,
      userProfile
    );

    // Generate reasoning
    const reasoning = this.generateWorkoutReasoning(
      userProfile,
      recommendedExercises
    );

    // Create alternatives and adaptations
    const alternatives = await this.generateWorkoutAlternatives(
      recommendedExercises,
      userProfile
    );
    ...
    return {
      ...
      description: this.generateWorkoutDescription(
        userProfile,
        recommendedExercises
      ),
  ...
  private calculateExerciseSimilarity(
    exercise1: Exercise,
    exercise2: Exercise
  ): number {
    let similarity = 0;
    ...
    similarity +=
      (muscleOverlap /
        Math.max(
          exercise1.muscleGroups.length,
          exercise2.muscleGroups.length
        )) *
      0.4;
  ...
  private calculateExerciseSuitability(
    _exercise: Exercise,
    constraints: Constraint[]
  ): number {
    let suitability = 1.0;
  ...
  private analyzeTimePerformance(
    ...
    timeMap.set("evening", 0.9);
    return timeMap;
  }

  private findOptimalWorkoutTimes(
    schedule: TimeSlot[],
    ...
    return schedule
      ...
      .sort((a, b) => {
        const aPerformance =
          performanceMap.get(this.getTimeOfDay(a.startTime)) || 0.5;
        const bPerformance =
          performanceMap.get(this.getTimeOfDay(b.startTime)) || 0.5;
  ...
  private generateOptimalWeeklyPlan(
    _optimalTimes: TimeSlot[],
    _performanceData: PerformanceMetrics[]
  ): WeeklyPlan {
    // Mock implementation
    const days: DayPlan[] = [
      {
        day: "Monday",
        type: "workout",
        reasoning: "High energy after weekend rest",
      },
      { day: "Tuesday", type: "rest", reasoning: "Recovery day" },
      {
        day: "Wednesday",
        type: "workout",
        reasoning: "Mid-week training session",
      },
      {
        day: "Thursday",
        type: "active-recovery",
        reasoning: "Light movement for recovery",
      },
    ...
    return [
      {
        ...
        implementation:
          "Elimina ejercicios accesorios y enfócate en compuestos",
      },
      {
  ...
  private analyzeFatigueTrends(fatigueMarkers: PerformanceMetrics[]): any {
    ...
    const avgFatigue =
      fatigueMarkers.reduce((sum, m) => sum + m.fatigue, 0) /
      fatigueMarkers.length;
  ...
  private generateDeloadModifications(
    fatigueAnalysis: any
  ): DeloadModification[] {
    ...
    if (fatigueAnalysis.severity === "high") {
      modifications.push({
        ...
        reduction: 50,
        explanation:
          "Reducir volumen significativamente para permitir recuperación completa",
      });
      modifications.push({
        parameter: "intensity",
        reduction: 30,
        explanation:
          "Bajar intensidad para reducir estrés del sistema nervioso",
      });
    } else {
      modifications.push({
        parameter: "volume",
        reduction: 30,
        explanation:
          "Reducción moderada de volumen manteniendo patrones de movimiento",
      });
    }
  ...
  private recommendDeloadActivities(_fatigueAnalysis: any): string[] {
    return [
  ...
  private async prescribeExercises(
    exercises: Exercise[],
    userProfile: UserProfile
  ...
  private calculateStartingWeight(
    _exercise: Exercise,
    _userProfile: UserProfile
  ): string {
  ...
  private generateExerciseProgression(
    _exercise: Exercise,
    _userProfile: UserProfile
  ): ExerciseProgression {
    return {
  ...
  private calculateWorkoutDifficulty(
    ...
    const avgExerciseDifficulty =
      exercises.reduce((sum, ex) => sum + ex.exercise.difficulty, 0) /
      exercises.length;
  ...
  private generateWorkoutReasoning(
    ...
    reasoning.push(
      `Entrenamiento diseñado para tu nivel: ${userProfile.experienceLevel}`
    );
    reasoning.push(
      `Enfocado en tus objetivos: ${userProfile.goals.join(", ")}`
    );
  ...
  private async generateWorkoutAlternatives(
    _exercises: RecommendedExercise[],
    _userProfile: UserProfile
  ): Promise<WorkoutAlternative[]> {
    return [
      {
        reason: "Si tienes menos tiempo disponible",
        modifications: [
          "Reducir descansos a 60 segundos",
          "Eliminar último ejercicio accesorio",
        ],
      },
      {
        reason: "Si no tienes acceso a todo el equipamiento",
        modifications: [
          "Usar ejercicios con peso corporal",
          "Sustituir con ejercicios de equipamiento disponible",
        ],
      },
    ];
  }

  private generateWorkoutAdaptations(
    _userProfile: UserProfile
  ): WorkoutAdaptation[] {
    return [
      {
  ...
  private generateWorkoutName(
    ...
    if (
      ...
      exercises.some((ex) => ex.exercise.compound)
    ) {
