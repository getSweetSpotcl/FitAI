import { AIResourceManager, UserProfile, CACHE_STRATEGY } from './ai-cost-control';
...
export class AIRoutineGenerator {
  ...
  async generateRoutine(request: RoutineGenerationRequest): Promise<{
    ...
    try {
      ...
      // 2. Verificar cache
      const cacheKey = this.aiManager.generateRoutineCacheKey(request.userProfile);
   ...
   */
  private async callOpenAI(request: RoutineGenerationRequest): Promise<GeneratedRoutine | null> {
...
Genera una rutina completa que sea progresiva, segura y efectiva para este perfil.`;
  ...
  private validateAndFormatRoutine(routine: any): GeneratedRoutine | null {
    try {
      ...
      const formattedExercises: GeneratedExercise[] = routine.exercises.map((ex: any) => ({
      ...
      return {
        ...
        difficulty: ['beginner', 'intermediate', 'advanced'].includes(routine.difficulty) 
          ? routine.difficulty 
          : 'intermediate',
   ...
   */
  private generateFallbackRoutine(request: RoutineGenerationRequest): GeneratedRoutine {
    const { userProfile } = request;
