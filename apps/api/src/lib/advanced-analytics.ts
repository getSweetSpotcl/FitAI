...
export class AdvancedAnalytics {
  ...
  async detectTrainingPlateaus(userId: string): Promise<PlateauPrediction[]> {
    ...
    for (const [exerciseId, performances] of Object.entries(exerciseGroups)) {
      const prediction = this.predictPlateauForExercise(exerciseId, performances);
      if (prediction) {
   ...
   */
  async calculateOptimalVolume(userId: string, userProfile: any): Promise<VolumeRecommendation> {
    const userWorkouts = this.workoutHistory.filter(w => w.userId === userId);
    ...
    const muscleGroupBreakdown = this.calculateMuscleGroupVolume(recentWorkouts);
   ...
   */
  async assessInjuryRisk(userId: string, patterns: MovementPattern[]): Promise<RiskAssessment> {
    const userWorkouts = this.workoutHistory.filter(w => w.userId === userId);
    ...
    if (userWorkouts.length < 3) {
      return {
        overallRisk: 'low',
        riskFactors: [],
        preventiveActions: ['Complete más entrenamientos para análisis completo'],
    ...
    return {
      ...
      confidenceScore: this.calculateConfidenceScore(userWorkouts.length, riskFactors.length)
    };
  }
  ...
  calculateTrainingStressScore(workout: WorkoutHistory): number {
    ...
    if (workout.heartRateData && workout.heartRateData.length > 0) {
      const avgHR = workout.heartRateData.reduce((sum, hr) => sum + hr.value, 0) / workout.heartRateData.length;
      heartRateFactor = Math.min(avgHR / 150, 1.5); // Normalize to max HR
    }

    return Math.round(durationFactor * volumeFactor * intensityFactor * heartRateFactor * 100);
  }

  /**
   * One-rep max estimation using multiple algorithms
   */
  estimateOneRepMax(exerciseId: string, recentSets: SetPerformance[]): number {
    ...
    recentSets.forEach(set => {
      if (set.reps >= 1 && set.reps <= 10) {
        ...
        // Lombardi formula
        const lombardi = set.weight * Math.pow(set.reps, 0.10);
    ...
    return Math.round(estimates.reduce((sum, est) => sum + est, 0) / estimates.length);
  }

  // Private helper methods

  private groupExercisesByType(workouts: WorkoutHistory[]): Record<string, ExercisePerformance[]> {
  ...
  private predictPlateauForExercise(exerciseId: string, performances: ExercisePerformance[]): PlateauPrediction | null {
    ...
    return {
      ...
      currentTrend: trend.slope > 0.1 ? 'increasing' : trend.slope > -0.05 ? 'stable' : 'decreasing',
      recommendations: this.generatePlateauRecommendations(likelihood, trend),
      confidence: this.calculatePredictionConfidence(recentPerformances.length, trend.rSquared)
    };
  }

  private calculateAverageVolume(workouts: WorkoutHistory[]): number {
    if (workouts.length === 0) return 0;
    return workouts.reduce((sum, w) => sum + w.totalVolume, 0) / workouts.length;
  }

  private calculateVolumeTrend(workouts: WorkoutHistory[]): { slope: number; rSquared: number } {
  ...
  private calculateRecoveryScore(workouts: WorkoutHistory[]): number {
    ...
    if (recoveryScores.length === 0) {
      ...
      const avgRPE = workouts.reduce((sum, w) => sum + w.avgRPE, 0) / workouts.length;
  ...
  private calculateBaseVolumeRecommendation(
    currentVolume: number,
    volumeTrend: { slope: number; rSquared: number },
    recoveryScore: number,
    ...
    return {
      ...
      reasoning: reasoning || 'Mantener volumen actual basado en análisis de datos.'
    };
  }

  private calculateMuscleGroupVolume(workouts: WorkoutHistory[]): Record<string, number> {
  ...
  private recommendPeriodization(volumeTrend: any, adaptationRate: number): 'linear' | 'undulating' | 'block' {
  ...
  private assessVolumeRisk(workouts: WorkoutHistory[]): RiskFactor | null {
    ...
    if (volumeTrend.slope > 0.3) { // Rapid volume increase
      return {
        type: 'volume',
        severity: 'high',
  ...
  private assessMovementPatternRisks(patterns: MovementPattern[]): RiskFactor[] {
  ...
  private calculateOverallRisk(riskFactors: RiskFactor[]): 'low' | 'moderate' | 'high' {
    if (riskFactors.length === 0) return 'low';

    const avgLikelihood = riskFactors.reduce((sum, rf) => sum + rf.likelihood, 0) / riskFactors.length;
    const highSeverityCount = riskFactors.filter(rf => rf.severity === 'high').length;
  ...
  private generatePreventiveActions(riskFactors: RiskFactor[]): string[] {
    ...
    if (hasIntensityRisk) {
      actions.push('Incluye más entrenamientos de intensidad moderada (RPE 6-7)');
    ...
    return actions.length > 0 ? actions : ['Mantener patrón actual de entrenamiento'];
  }

  private generateMonitoringPoints(riskFactors: RiskFactor[]): string[] {
    return [
  ...
  private calculateConfidenceScore(workoutCount: number, riskFactorCount: number): number {
  ...
  private calculateLinearRegression(points: { x: number; y: number }[]): { slope: number; rSquared: number } {
  ...
  private getTimeSinceLastPR(performances: ExercisePerformance[]): number {
    // Mock implementation - would track actual PRs in production
    return Math.floor(Math.random() * 8) + 1;
  }

  private generatePlateauRecommendations(likelihood: number, trend: any): string[] {
  ...
  private calculatePredictionConfidence(dataPoints: number, rSquared: number): number {
